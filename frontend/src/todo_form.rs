...
pub fn todo_form(props: &TodoFormProps) -> Html {
    ...
    html! {
        <form id="todoForm" class="todo-form" onsubmit={on_add}>
            <input type="text" name="todoInput" id="todoInput" placeholder="What needs to be done?" required=true />
            <div class="button-group">
                <button type="submit">{"Add"}</button>
                <button type="button" id="clearCompleted" class="clear-btn" aria-label="Delete completed tasks" onclick={on_clear}>
